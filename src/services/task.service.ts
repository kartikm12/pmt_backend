...
import { USER_ROLE, TASK_STATUS, type UserRole } from "../constants/enums.js";
...
export class TaskService {
  static async list(userId: string, role: UserRole, projectId?: string, page = 1, limit = 50) {
  ...
  static async getById(taskId: string, userId?: string, role?: UserRole) {
  ...
  static async update(
    ...
    const existingTask = await this.getById(taskId, actorUserId, userRole as UserRole);
    const isManager = ProjectService.isManagerRole(userRole as UserRole);
    ...
    const updatedTask = await prisma.task.update({
      ...
      data: {
        ...
        startedAt:
          input.status === TASK_STATUS.IN_PROGRESS && !existingTask.startedAt ? new Date() : undefined,
        completedAt
      }
    });
  ...
  static async remove(taskId: string, actorUserId: string) {
