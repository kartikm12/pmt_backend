import { MEMBERSHIP_ROLE, PROJECT_STATUS, USER_ROLE } from "../constants/enums.js";
...
export class ProjectService {
    ...
    static async list(userId, role, page = 1, limit = 50) {
    ...
    static async getById(projectId, userId, role) {
    ...
    static async getBundle(projectId, userId, role) {
        const isManager = !userId || !role || this.isManagerRole(role);
        const project = await prisma.project.findFirst({
            where: {
                id: projectId,
        ...
        members.forEach((m) => usersMap.set(m.user.id, m.user));
        tasks.forEach((t) => {
            t.assignees.forEach((a) => usersMap.set(a.id, a));
        });
        return {
            project: {
                ...projectData,
    ...
    static async create(userId, userRole, input) {
    ...
    static async update(projectId, userId, userRole, input) {
        ...
        if (input.memberIds) {
            ...
            const currentMemberIds = existingMembers.map((m) => m.userId);
    ...
    static async archive(projectId, userId, userRole) {
    ...
    static async assignMembers(projectId, actorUserId, userRole, input) {
    ...
    static async removeMember(projectId, memberId, actorUserId, userRole) {
