...
export class ReportsService {
    static async getComprehensiveReport(userId, role, filters = {}) {
        const daysCount = filters.days || 7;
        const isManager = MANAGER_ROLES.includes(role);
        const taskBaseWhere = { deletedAt: null };
        const projectBaseWhere = { deletedAt: null };
        ...
        const trends = days.map((day) => {
            ...
            for (const t of tasksInRange) {
                if (t.createdAt >= day.start && t.createdAt <= day.end)
                    created++;
                if (t.status === TASK_STATUS.DONE && t.completedAt && t.completedAt >= day.start && t.completedAt <= day.end)
                    completed++;
            }
            return {
                name: day.label,
        ...
        for (const task of allRelevantTasks) {
            for (const assignee of task.assignees) {
                ...
                if (filters.memberId && assignee.id !== filters.memberId)
                    continue;
