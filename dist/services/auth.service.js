...
export class AuthService {
    static async register(input, metadata) {
        ...
        return { user: { ...userWithoutPassword, roleLocked: user.roleLocked }, token: session?.token ?? null };
    }
    static async login(input, metadata) {
        ...
        await socketService.forceLogoutSessions(session.replacedSessionIds, "Your account was logged in from another device.");
        return {
            token: session.token,
            user: {
                id: user.id,
    ...
    static async me(userId) {
        ...
        return { ...rest, roleLocked: user.roleLocked, memberships: mergedMemberships };
    }
